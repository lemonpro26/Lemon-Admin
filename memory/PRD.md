...
## Implemented (2026-07-20b) — Leads tab UX
- **Leads table horizontal-scroll preservation**: clicking the ◀/▶ date arrows (or a segment/campaign chip) no longer snaps the table back to the left, hiding the Date column. The `<div className="overflow-x-auto">` scroll container is now ALWAYS mounted (loading / empty-state placeholders render *inside* it), and a `leadsScrollRef` + `useLayoutEffect` restore `scrollLeft` on every `leads` update. File: `frontend/src/pages/AdminDashboard.jsx`.
- **Grouped filter panels**: Landing page, Campaign, and Network filters each live in their own bordered card (rounded-2xl, border-slate-200, shadow-sm) with an all-caps section label, so the three orthogonal filter axes read as separate controls instead of a stacked chip wall. File: `frontend/src/pages/AdminDashboard.jsx`.


## Implemented (2026-07-20c) — Resizable columns
- **Draggable column widths on the Calls table**: added `useResizableColumns` hook (`frontend/src/lib/useResizableColumns.js`) + `ColResizeHandle` component. Every column header (Caller / Number / Called # / Duration / Campaign / Hook / Revenue / Location / When / Actions) has a thin drag handle on its right edge; hover it and the cursor becomes `col-resize`, dragging updates the width live. A `<colgroup>` with per-column `<col style={{ width }}>` drives the layout. Widths persist to localStorage (`lp_calls_col_widths_v1`) so the layout survives refreshes. "Reset column widths" was added to the Columns dropdown. Verified: dragging Caller +120px correctly widened Caller and compressed other columns.
